...
public class TryAI extends AIModule {
	ArrayList<Point> availMoves;
	HashMap<Point, Integer> played;
	private int player;

	public void getNextMove(final GameStateModule game) {
		int i = 1;
		played = new HashMap<Point, Integer>();
	...
	private void collectCoins(final GameStateModule game) {
		
		for (int i = 0; i < game.getWidth(); i++) {
			for (int j = 0; j < game.getHeight(); j++) {
				if (game.getAt(i, j) != 0) {
					played.put(new Point(i, j), game.getAt(i, j));
				}
			}
		}
	...
	private int maxVal(final GameStateModule game, int depth, int alpha, int beta, Point p) {
		...
		for (int t = 0; t < Arr.size(); t++) {
			if(!terminate){
			game.makeMove((int)  Arr.get(t).getX());
			played.put( Arr.get(t), game.getActivePlayer());
...
//need to check for terminal flag. this means..?
